...
def getDataFromDB(deviceId, moduleId):
	...
	where_stm = 'WHERE device_id = ' + str(deviceId) + ' AND module_id = ' + str(moduleId) + ' AND ts BETWEEN DATE_SUB(NOW(), INTERVAL 24 DAY) AND NOW()'
...
def train(moduleId):
	...
	dbData = getDataFromDB(deviceId, moduleId)
	try:
	...
	"""
	rnaArray.update( {moduleId: clf_model_state } )
	return {
