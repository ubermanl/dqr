...
void acRead(int rZero, int sampleDuration) {
  ...
  double voltsError = (rZero * 5) / 1023.0;
  double voltRMS = 5.0 * sqrt(rSquaredSum / sampleCount) / 1023.0;
  //Serial.print(rSquaredSum);
  //Serial.print(" rSquaredSum | ");
  //Serial.print(sampleCount);
  //Serial.print(" sampleCount | ");
  //Serial.println(sqrt(rSquaredSum / sampleCount));
  Serial.println(rSquaredSum / sampleCount);
...
void loop(){
  ...
  if (touchCurrent == HIGH) {
   ...
   //delay(20);
   int iZero = 516;
   double startTime=millis(); // OJO con el millis y el tipo de variable! (llega un momento que millis no entra en int)
   float max = 0;
   int rVal = 0;
   while((millis()-startTime) < 200)
  {
    rVal = analogRead(ACSENSOR_INPUT) - iZero;
    if (abs(rVal) > max) {
      max = abs(rVal);
    }
  }
